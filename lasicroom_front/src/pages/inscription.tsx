...
function Inscription() {
  ...
  const [erreurs, setErreurs] = useState<ErrorsType>({});
  ...
  const gererSoumission = async (e: FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      if (reponse.ok && donnees.utilisateur) {
        ...
        setErreurs({
          global: donnees.message || "Erreur lors de l’inscription.",
        });
      }
    } catch (err) {
    ...
    <div>
      ...
      <main className="display_main">
        <div className="div_form">
          <h2 className="style_h2" style={{ textAlign: "center" }}>
            Inscription
          </h2>
          <form className="style_form" onSubmit={gererSoumission}>
            ...
            />
            {erreurs.motDePasse && (
              <p style={{ color: "red" }}>{erreurs.motDePasse}</p>
            )}
